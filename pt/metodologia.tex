...
As bibliotecas de Python usadas foram pandas, polars, matplotlib, scikit-learn, geopandas e cartopy. A escolha do polars ao invés do pandas foi motivada pela  sua API moderna construída com Rust do polars. Essa tem um poder de processamento de dados mais potente do que o tradicional pandas.
...
O matplotlib foi usado para a geração e customização das figuras no nível do eixo. A biblioteca define o tamanho das figuras pela largura e altura, respectivamente, em polegadas. Um polegada equivale a 2,54 cm. O tamanho padrão das figuras é 10x5 polegadas. Caso seja necessário usar outro tamanho, ele será adotado.
...
A subseção será dividida em duas sub-subseções. A sub-subseção \ref{metodologia_conj_dados_sermil} tratará dos dados do serviço militar; já a \ref{metodologia_conj_dados_ibge} tratará dos dados das projeções populacionais.

\subsubsection{Serviço militar}
\label{metodologia_conj_dados_sermil}
...
\begin{longtable}{cccc}
	...
	\toprule
	\textbf{Ano} & \textbf{Peso} & \textbf{Altura} & \textbf{Dispensa} \\
	\midrule
	\endfirsthead
...
Como a quantidade de alistados solteiros é 97,55\% e os demais representam insignificantemente 2,45\% to total, apenas os alistados solteiros foram selecionados e coluna ESTADO\_CIVIL foi removida. A quantidade de linhas foi reduzida para 25.298.928.
...
Para a coluna ZONA\_RESIDENCIAL, que contém apenas os valores "Rural" e "Urbano", descobriu-se que, no geral, 87,89\% dos alistados residem na zona urbana, enquanto 12,11\% vivem na zona rural. Como não houve necessidade de remoção, ZONA\_RESIDENCIAL foi transformada no tipo categórico. 

A última coluna a ser tratada é ESCOLARIDADE. Os graus de escolaridade, como constam no dataframe, estão listados na tabela \ref{tab:escolaridade}. As inconsistências foram corrigidas e o texto está correto.

Conforme demonstrado no listing \ref{lst:processamento_escolaridade}, cada grau escolar foi avaliado para checar de qual grupo de graus de escolaridade está incluso. Cada rótulo passou a ser parte de uma nova coluna chamada de grau\_educacional, o que levou para a remoção da coluna ESCOLARIDADE.

Com todas as colunas tratadas, os títulos delas foram uniformizados, pois o que estavam em maiúsculo foram transformados em títulos minúsculos. Como resultado, a etapa de ETL e data wrangling foi encerrada.

Os detalhes da etapa da análise exploratória e do aprendizado de máquina serão discutidas na seção \ref{discussao}.

\subsubsection{Projeções populacionais}
\label{metodologia_conj_dados_ibge}
